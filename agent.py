...
2. Finds rides with pickup time ~30 minutes from now
...
# Initialize colorama for colored console output on Windows
init(autoreset=True)
...
def signal_handler(sig, frame):
    ...
    global running
    print(f"\n{Fore.YELLOW}⚠  Shutdown signal received. Stopping agent...{Style.RESET_ALL}")
    running = False
...
{Fore.CYAN}╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║   {Fore.WHITE}🚕  Mr. Cabie — Driver Pickup Reminder Agent (v1){Fore.CYAN}          ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
    print(banner)
...
def print_config():
    ...
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  Excel File     : {Fore.WHITE}{config.EXCEL_FILE_PATH}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  Webhook URL    : {Fore.WHITE}{config.RENDER_WEBHOOK_URL}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  Check Interval : {Fore.WHITE}{config.CHECK_INTERVAL_SECONDS}s{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  Reminder Before: {Fore.WHITE}{config.REMINDER_MINUTES_BEFORE} minutes{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  Twilio Number  : {Fore.WHITE}{config.TWILIO_PHONE_NUMBER}{Style.RESET_ALL}")
...
def process_ride(ride):
    ...
    print(f"\n{Fore.YELLOW}  📞 Calling {Fore.WHITE}{driver_name}{Fore.YELLOW} at {Fore.WHITE}{driver_phone}{Style.RESET_ALL}")
    print(f"     Pickup: {pickup_location} at {pickup_time_str}")
    ...
    if not call_sid:
        ...
        print(f"     {Fore.RED}✗ Call failed to initiate{Style.RESET_ALL}")
        save_call_log(log_entry)
        return log_entry

    print(f"     {Fore.CYAN}⏳ Call initiated (SID: {call_sid[:20]}...){Style.RESET_ALL}")

    # Step 2: Wait for call to complete
    print(f"     {Fore.CYAN}⏳ Waiting for call to complete...{Style.RESET_ALL}")
    ...
    if final_status == "completed":
        print(f"     {Fore.GREEN}✓ Call completed — Reminder sent!{Style.RESET_ALL}")
    elif final_status == "no-answer":
        print(f"     {Fore.RED}✗ No answer — Logged{Style.RESET_ALL}")
    elif final_status == "busy":
        print(f"     {Fore.RED}✗ Line busy — Logged{Style.RESET_ALL}")
    else:
        print(f"     {Fore.RED}✗ Call {final_status} — Logged{Style.RESET_ALL}")
...
def run_check_cycle(cycle_count):
    ...
    print(f"  {Fore.WHITE}📋 Scanning Excel for pending rides...{Style.RESET_ALL}")
    pending_rides = get_pending_rides()

    if not pending_rides:
        print(f"  {Fore.WHITE}😴 No rides due for reminder right now.{Style.RESET_ALL}")
        return
...
def main():
    ...
    try:
        ...
        print(f"{Fore.GREEN}  ✓ Configuration validated successfully{Style.RESET_ALL}\n")
    ...
    print(f"{Fore.GREEN}  🚀 Agent started! Checking every {config.CHECK_INTERVAL_SECONDS} seconds.{Style.RESET_ALL}")
    ...
    while running:
        ...
        try:
            ...
            print(f"\n  {Fore.RED}[ERROR] Cycle #{cycle_count} failed: {e}{Style.RESET_ALL}")

        # Wait for next cycle (check every second so we can respond to Ctrl+C quickly)
        for _ in range(config.CHECK_INTERVAL_SECONDS):
            if not running:
                break
            time.sleep(1)

    print(f"\n{Fore.CYAN}  👋 Agent stopped. Goodbye!{Style.RESET_ALL}\n")
